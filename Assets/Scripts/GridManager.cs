...
public class GridManager: MonoBehaviour
{
    ...
    */
    ...
    public LayerMask obstacleMask;  // what layers are obstacles?  (1)
    ...
    public void CreateGrid()  //(4)
    {
        ...
        for(int x = 0; x < gridSizeX; x++)
        {
            for(int y = 0; y < gridSizeY; y++)
            {
                ...
                //using same concept as we did for the melee combat
                if (Physics2D.OverlapCircle(worldPoint, nodeRadius / 2, obstacleMask) != null)
                {
                    isWalkable = false;
